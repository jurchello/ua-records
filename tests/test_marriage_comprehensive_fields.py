...
def test_marriage_form_all_person_fields_with_helper():
    """Test all person fields using the comprehensive helper."""
    tester = create_marriage_field_tester()
...
def test_marriage_form_identify_missing_model_paths():
    """Identify fields that cannot be tested due to missing model_path."""
    tester = create_marriage_field_tester()
    ...
    for field in untestable:
        ...
        if not model_path or '.' not in model_path:
            no_model_path.append(f"{field_id} ({field_type})")
        elif field_type not in tester.get_testable_field_types():
    ...
    if no_model_path:
        print(f"Fields without model_path: {no_model_path[:5]}...")
...
def test_marriage_form_generate_field_type_test_values():
    ...
    for field_type, expected_type in test_cases.items():
        dummy_field = {'id': f'test_{field_type}', 'type': field_type}
...
if __name__ == "__main__":
    ...
    test_marriage_form_identify_missing_model_paths()
    print("✓ Missing model_path analysis passed")
