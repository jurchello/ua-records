...
def test_work_context_form_state_lifecycle():
    ...
    class TrackingWorkContext(MockWorkContext):
        def reset(self):
            form_state_snapshots.append({
                'action': 'reset',
                'old_form_state': self.form_state,
                'old_db': self.db
            })
            super().reset()
    ...
    assert form_state_snapshots[0]['old_form_state'].to_dict() == {"test_field": "test_value"}, "Старий стан правильно збережений"
