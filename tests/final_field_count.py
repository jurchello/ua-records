...
def test_final_field_breakdown():
    ...
    sections = {"common_box": [], "groom_box": [], "bride_box": [], "clergymen_box": []}
    ...
    for field in all_fields:
        ...
        for section_name in sections.keys():
            if field_id.startswith(section_name + "."):
                sections[section_name].append({"id": field_id, "type": field_type})
                break
...
def analyze_person_box(fields, box_name):
    ...
    for field in fields:
        ...
        components[component].append(
            {"id": field_id, "type": field_type, "suffix": ".".join(parts[1:]) if len(parts) > 1 else ""}
        )
...
def analyze_clergy_box(fields):
    ...
    for field in fields:
        ...
        components[component].append({"id": field_id, "type": field_type})
