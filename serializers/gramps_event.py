...
def _normalize_date(d: Any) -> Dict[str, Any]:
    ...
    if callable(gs) and callable(ge):
        ...
        span = {
            "start": (
                {
                    "yyyy": s[2] if s and len(s) > 2 else None,
                    "mm": s[1] if s and len(s) > 1 else None,
                    "dd": s[0] if s and len(s) > 0 else None,
                }
                if s
                else {}
            ),
            "end": (
                {
                    "yyyy": e[2] if e and len(e) > 2 else None,
                    "mm": e[1] if e and len(e) > 1 else None,
                    "dd": e[0] if e and len(e) > 0 else None,
                }
                if e
                else {}
            ),
...
def serialize_event(e: Any) -> Dict[str, Any]:
    ...
    for a in getattr(e, "get_attribute_list", lambda: [])() or []:
        attrs.append(
            {
                "type": _norm_str(getattr(a, "get_type", lambda: "")()),
                "value": _norm_str(getattr(a, "get_value", lambda: "")()),
            }
        )
    return {
