...
def deep_diff(a: Any, b: Any, *, id_keys: Tuple[str,...]=("handle","id","gid")) -> Any:
    if a == b:
        return {}
    ...
    if isinstance(a, list) and isinstance(b, list):
        if all(not isinstance(x, (dict, list)) for x in a+b):
            ...
            if rem: diff["removed"] = rem
            if add: diff["added"] = add
            return diff or {}
        ...
        if rem: map_diff["removed"] = rem
        if add: map_diff["added"] = add
        chg: Dict[str, Any] = {}
        for k in sorted(ma.keys() & mb.keys()):
            d = deep_diff(ma[k], mb[k], id_keys=id_keys)
            if d != {}:
                chg[k] = d
        if chg: map_diff["changed"] = chg
        if ra or rb:
            sa, sb = set(map(str, ra)), set(map(str, rb))
            if sa - sb: map_diff.setdefault("removed", []).extend(sorted(sa - sb))
            if sb - sa: map_diff.setdefault("added",  []).extend(sorted(sb - sa))
        return map_diff or {}
    return {"from": a, "to": b}