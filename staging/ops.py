...
def build_change_ops(baseline: Dict[Tuple[str,str], Dict[str,Any]], after: Dict[Tuple[str,str], Dict[str,Any]]) -> List[ChangeOp]:
    ops: List[ChangeOp] = []
    ...
    for k in sorted(after_keys - base_keys):
        kind, oid = k
        ref = ObjKey(kind, oid)
        ops.append(ChangeOp(id=_hash_id("create",kind,oid), section="new", ref=ref, path="", before=None, after=after[k], op_type="create_object"))
        # field-level sets
        flat = _flatten(after[k])
        for p, v in flat.items():
            ops.append(ChangeOp(id=_hash_id("new",kind,oid,p), section="new", ref=ref, path=p, before=None, after=v, op_type="set"))

    # object-level deletes
    for k in sorted(base_keys - after_keys):
        kind, oid = k
        ref = ObjKey(kind, oid)
        ops.append(ChangeOp(id=_hash_id("delete",kind,oid), section="deleted", ref=ref, path="", before=baseline[k], after=None, op_type="delete_object"))
    ...
    for k in sorted(base_keys & after_keys):
        ...
        for p in keys:
            ...
            if va is None and vb is not None:
                ...
                opt = "replace"
            ops.append(ChangeOp(id=_hash_id("mod",kind,oid,p), section="modified", ref=ref, path=p, before=va, after=vb, op_type=opt))
    return ops